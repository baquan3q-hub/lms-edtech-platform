...
export default function ParentProgressClient({ students, activeStudentId, activeStudentName, stats, history, feedbackList, competencyData, pointsData }: ParentProgressClientProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* ===================== BIỂU ĐỒ QUÁ TRÌNH HỌC TẬP (LINE CHART) ===================== */}
            {history.length > 0 && (
                <Card className="shadow-sm overflow-hidden">
                    <CardHeader className="bg-gradient-to-r from-indigo-50 to-sky-50 border-b border-indigo-100">
                        <CardTitle className="flex items-center gap-2">
                            <TrendingUp className="w-5 h-5 text-indigo-600" /> Quá trình Học tập
                        </CardTitle>
                        <CardDescription>
                            Biểu đồ điểm số qua các bài kiểm tra và bài tập theo thời gian
                        </CardDescription>
                    </CardHeader>
                    ...
                    {competencyData.skills.length > 0 && (
                        <Card className="lg:col-span-3 shadow-sm overflow-hidden">
                            ...
                            <CardContent className="pt-6 pb-2">
                                ...
                                {/* Legend */}
                                <div className="flex flex-wrap items-center justify-center gap-4 mt-3 pb-2 text-xs text-slate-500">
                                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-emerald-500"></span> ≥80% Xuất sắc</span>
                                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-indigo-500"></span> 60-79% Tốt</span>
                                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-amber-500"></span> 40-59% TB</span>
                                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-red-500"></span> &lt;40% Yếu</span>
                                </div>
                            </CardContent>
                        </Card>
                    )}
                                    ...
                                    <div className="space-y-2">
                                        {competencyData.weaknesses.map(w => (
                                            <div key={w.key} className="flex items-center gap-3 p-3 bg-amber-50/70 rounded-xl border border-amber-100">
                                                ...
                                                <Badge className="bg-amber-100 text-amber-700 border-none text-xs shrink-0">
                                                    ⚠️ Cần ôn
                                                </Badge>
                                            </div>
                                        ))}
